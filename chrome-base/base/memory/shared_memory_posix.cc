  ...
  if (fp == NULL) {
    ...
    if (access(dir.value().c_str(), W_OK | X_OK) < 0) {
      ...
      if (dir.value() == "/dev/shm") {
        GLOG(FATAL) << "This is frequently caused by incorrect permissions on "
...
bool SharedMemory::PrepareMapFile(ScopedFILE fp, ScopedFD readonly_fd) {
  ...
  if (readonly_fd.is_valid()) {
    ...
    if (st.st_dev != readonly_st.st_dev || st.st_ino != readonly_st.st_ino) {
      GLOG(ERROR) << "writable and read-only inodes don't match; bailing";
      return false;
    }
  }
  ...
  if (mapped_file_ == -1) {
    if (errno == EMFILE) {
      GLOG(WARNING) << "Shared memory creation failed; out of file descriptors";
      return false;
    } else {
