...
bool InitializeICUWithFileDescriptorInternal(
  ...
  if (data_fd == kInvalidPlatformFile) {
    GLOG(ERROR) << "Invalid file descriptor to ICU data received.";
    return false;
  }
  ...
  if (!icudtl_mapped_file->Initialize(File(data_fd), data_region)) {
    GLOG(ERROR) << "Couldn't mmap icu data file";
    return false;
  }
...
bool InitializeICU() {
  ...
  if (!module) {
    GLOG(ERROR) << "Failed to load " << ICU_UTIL_DATA_SHARED_MODULE_NAME;
    return false;
  }

  FARPROC addr = GetProcAddress(module, ICU_UTIL_DATA_SYMBOL);
  if (!addr) {
    GLOG(ERROR) << ICU_UTIL_DATA_SYMBOL << ": not found in "
